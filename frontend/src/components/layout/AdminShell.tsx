  ...
  const sidebar = (compact = false) => (
    <>
      ...
      <div className="border-t border-slate-200 p-3 dark:border-slate-800/80">
        ...
        <Button className={cn('h-[52px] w-full gap-3 text-sm text-slate-600 hover:bg-slate-100 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white', compact ? 'justify-center px-0' : 'justify-start px-4')} variant="ghost" onClick={logout} title={compact ? 'Log out' : undefined} aria-label="Log out"><span className="flex size-5 min-w-5 shrink-0 items-center justify-center leading-none" aria-hidden="true"><LogOut className="block size-[18px] shrink-0" /></span>{!compact && <span className={sidebarLabelClass}>Log out</span>}</Button>
      </div>
    </>
  );
